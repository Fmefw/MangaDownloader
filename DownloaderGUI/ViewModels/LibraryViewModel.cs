...
namespace DownloaderGUI.ViewModels
{
    ...
    public class Manga : ViewModelBase
    {
        ...
        public ObservableCollection<PdfFileInfo> PdfFiles { get; set; } = new ObservableCollection<PdfFileInfo>();
    }
    ...
    public class LibraryViewModel : ViewModelBase
    {
        ...
        private ObservableCollection<Manga> _filteredMangaLibrary; //create an observable collection with the filtered mangas from the query
        ...
        public ObservableCollection<Manga> MangaLibrary { get; } //Set an observable collection of mangas called "MangaLibrary" 
        ...
        public void ShowPdfList(Manga manga, string path)
        {
            if (Directory.Exists(path))
            {
                if (manga.PdfFiles.Count == 0)
                {
                    ...
                    foreach (var sortedPdf in sortedPdfFiles)
                    {
                        manga.PdfFiles.Add(sortedPdf);
                    }
                }
                else
                {
        ...
        public class ChapterComparer : IComparer<string>
        {
            public int Compare(string x, string y)
            {
                ...
                if (int.TryParse(x.Replace("Chapter ", ""), out int chapterX) &&
                    int.TryParse(y.Replace("Chapter ", ""), out int chapterY))
                {
